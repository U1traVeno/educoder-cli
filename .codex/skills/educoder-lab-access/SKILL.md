...
11. Run `update-file` when a repository file must be saved through Educoder's editor API without triggering evaluation.
12. Prefer `evaluate-file` when saving a file and triggering official evaluation in one browser-equivalent flow; it preserves the `sec_key`, `resubmit`, `content_modified`, and commit metadata returned by the save API.
