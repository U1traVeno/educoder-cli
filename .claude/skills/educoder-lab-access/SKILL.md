...
Educoder uses web terminal/WebSocket infrastructure and also returns an SSH gateway from `myshixuns/<id>/start.json` for these OS labs. Prefer `vm-exec` for Claude Code-driven terminal work because it hides the password and works from the local shell.
